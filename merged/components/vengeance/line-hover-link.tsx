...
/* Vengeance UI — line-hover-link (registry:ui), adapted.
   Source: https://github.com/Ashutoshx7/VengeanceUI  public/r/line-hover-link.json
   Commit 813d9c1 (2026-08-31). MIT licence — see ./LICENSE.

   Changes from upstream, all local to this copy:
   - an internal href ("/method") renders Next's <Link>, so navigation stays
     client-side and prefetched; external hrefs stay a plain <a>;
   - the stylesheet is emitted once per page through React 19's <style href
     precedence> de-duplication instead of once per link;
   - the focus ring keeps square corners, like the rest of this interface;
   - the rules sit in Tailwind's `components` cascade layer, so a utility on
     the link (`text-ochre`, `mb-4`) wins over the component's defaults — an
     unlayered <style> outranks every Tailwind v4 utility, colour included. */

import * as React from "react";
import Link from "next/link";
import { cn } from "@/lib/utils";

const lineHoverStyles = `@layer components {
.link-hover {
  cursor: pointer;
  position: relative;
  display: inline-flex;
...
>(({ variant = "slide", children, className, ...props }, ref) => {
    ...
    const classes = cn("link-hover", `link-hover--${variant}`, className);
    const inner = (
        <>
    ...
    return (
        <>
            <style href="vengeance-line-hover-link" precedence="default">{lineHoverStyles}</style>
            {internal ? (
                <Link ref={ref} href={href} className={classes} {...rest}>{inner}</Link>
            ) : (
                <a ref={ref} href={href} className={classes} {...rest}>{inner}</a>
            )}
        </>
    );
});
